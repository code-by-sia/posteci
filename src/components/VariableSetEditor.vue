    ...
    export default class VariableSetEditor extends Vue {
        ...
        shouldBlur(v: Variable) {
            return [
                ...
                'bearer',
                'basic'
            ].find(sensitive => v.name.indexOf(sensitive) > -1 || v.value.trim().startsWith(sensitive))
        }
    }
</script>
...
<template>
    <div class="variable-set-editor">
        <div class="variables">
            ...
            <div v-for="(variable,index) in variables" class="variable">
                <input type="text" v-model="variable.name" @change="onVariableChange">
                <input type="text" v-model="variable.value" @change="onVariableChange"
